      ...
      <div className={dark ? "min-h-screen bg-slate-900 text-slate-100" : "min-h-screen bg-slate-50 text-slate-900"}>
        ...
        <main className="max-w-7xl mx-auto grid grid-cols-12 gap-4 px-4 py-6">
          <aside className="col-span-12 md:col-span-3 lg:col-span-2">
            <nav className={dark ? "bg-slate-800 border border-slate-700 rounded-2xl p-2" : "bg-white border border-slate-200 rounded-2xl p-2 shadow-sm"}>
              {NAV.map((n) => (
                <button key={n.key} onClick={() => setRoute(n.key)}
                  className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-left ${route===n.key ? (dark?"bg-slate-700":"bg-slate-100") : (dark?"hover:bg-slate-700":"hover:bg-slate-50")}`}>
        ...
        <footer className="text-center text-xs opacity-60 py-6">© {new Date().getFullYear()} Sales Tracker – Local demo build</footer>
      </div>
    </ThemeCtx.Provider>
  );
}
